...
type View =
  | { name: "top" }
  | { name: "diagnosis"; pageIndex: number }
  | { name: "result"; resultType: FoodMbtiType }
  | { name: "typeDetail"; typeId: string };

const QUESTIONS_PER_PAGE = 5;

let currentView: View = { name: "top" };
let answers: Answer[] = [];
...
function setAnswer(questionId: string, choice: "A" | "B") {
...
function getAnswerChoice(questionId: string): "A" | "B" | null {
...
function renderDiagnosis(pageIndex: number) {
...
// 診断結果ページ
function renderResult(resultType: FoodMbtiType) {
  app.innerHTML = `
...
function render() {
  switch (currentView.name) {
    ...
    case "diagnosis": renderDiagnosis(currentView.pageIndex); break;
    case "result": renderResult(currentView.resultType); break;
    case "typeDetail": renderTypeDetail(currentView.typeId); break;
  }
}
